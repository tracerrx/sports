...
func (s *Weather) getSmallWriter(canvasBounds image.Rectangle) (*rgbrender.TextWriter, error) {
	s.Lock()
	defer s.Unlock()
...
func (s *Weather) getBigWriter(canvasBounds image.Rectangle) (*rgbrender.TextWriter, error) {
	s.Lock()
	defer s.Unlock()
